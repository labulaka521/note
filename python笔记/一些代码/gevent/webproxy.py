...
def application(env, start_response):
    ...
    if (method, path) == ('GET', ''):
        ...
        start_response(
            '302 Found',
            [('Location', _as_str(join(proxy_url, unquote(value))))])
    elif method == 'POST':
...
def proxy(path, start_response, proxy_url):
    ...
    try:
        ...
        error_str = '<h1>%s</h1><h2>%s</h2><pre>%s</pre>' % (
...
def fix_links(data, proxy_url, host_url):
    ...
    def fix_link_cb(m):
        ...
        if b'://' in url:
            ...
            result = m.group('before') + b'"' + join(proxy_url, host_url,
                                                     url) + b'"'
...
_link_re_1 = re.compile(
    br'''(?P<before>(href|src|action)\s*=\s*)(?P<quote>['"])(?P<url>[^#].*?)(?P=quote)'''
)
_link_re_2 = re.compile(
    br'''(?P<before>(href|src|action)\s*=\s*)(?P<url>[^'"#>][^ >]*)''')
